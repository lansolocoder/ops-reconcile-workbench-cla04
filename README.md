...
- 合并时每份 PATCH 内部仍不得重复目标单元格，但不同发现可以修改同一单元格。所有修改先按发现身份逐项升序，再按记录号、逻辑字段文本升序执行；同一单元格以后执行的值覆盖先前值，最终 CSV、重审计结果与 `DERIVED` 批次内容均以该值为准。随后用 `SOURCE` 的 schema 对修正后的 CSV 重新审计，审计 JSONL 写到 stdout 或 `--report R`。
- 修正后 CSV 写到 `--output`：保持表头、列序、额外列及所有未改单元格（含引号包裹的逗号与字段内换行），UTF-8 无 BOM、统一 `\n` 行结束；`INPUT` 始终不变。
- 单事务在 `derived_batches` 表以 `DERIVED` 保存新哈希、schema、发现集、`SOURCE` 批次 ID 以及决定/提案快照。快照按发现身份升序，逐项保存决定的 `action`、修剪后的 `reason`、决定绑定的完整原发现，以及提案绑定的完整原发现与 PATCH（均取自存储内容，不省略、不从当前发现反推）。同一 `DERIVED` 且内容完全相同幂等返回 0（仍重新生成输出）；内容不同退出 3 且不触碰旧输出。
- 退出 0 表示应用与重审计成功（无论重审计是否还有发现，审计结果以 JSONL 的 summary 为准）。失效决定、非法修正以及读写、数据库、审计错误一律退出 2：数据库与 `DERIVED` 批次不变。替换前先保存每个输出目标（CSV 及可选报告）的原有字节与“原先不存在”状态，再完整暂存并依次原子替换；任一暂存、替换或事务提交失败都回滚 `DERIVED`，并把两个输出都恢复到调用前的字节或不存在状态——已成功替换的也必须撤回——随后清理本次临时文件与备份。若恢复本身也失败，stderr 同时说明原失败与恢复失败，绝不宣称成功。
